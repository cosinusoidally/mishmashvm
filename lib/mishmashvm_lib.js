...
mm.load_c_string=function(x,options){
  mm.writeFile(mm.cfg.tmpdir+"/tmp.c",x);
...
mm.writeFile=function(file,data){
  if(!mm.fwrite){
    if(mm.libc_compat){
      mm.libc_compat_linked=mm.link([mm.libc_compat]);
      mm.fwrite=mm.libc_compat_linked.get_fn("fwrite");
    };
  };
  if(mm.fwrite){
    var f=libc.fopen(file,"wb");
    mm.fwrite(data,data.length,1,f);
    libc.fclose(f);
  } else {
