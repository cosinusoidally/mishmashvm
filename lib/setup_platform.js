...
kernel32.VirtualAlloc=kernel32.lib.declare("VirtualAlloc",ctypes.default_abi,ctypes.uint32_t,ctypes.voidptr_t,ctypes.uint32_t,ctypes.uint32_t,ctypes.uint32_t);
...
print("win32 setup complete");
    ...
    print("Platform setup failed, please read README");
    quit();
  }
}
...
libc.puts=libc.lib.declare("puts",ctypes.default_abi,ctypes.uint32_t, ctypes.char.ptr);


// DANGER on Linux we might get malloc from Spidermonkey and realloc/free
// from glibc. The below declarations seem to work, but I'd be very wary
// about using them without further investigation.

libc.realloc=libc.lib.declare("realloc",ctypes.default_abi, ctypes.uint32_t,ctypes.uint32_t,ctypes.uint32_t);
libc.free=libc.lib.declare("free",ctypes.default_abi, ctypes.uint32_t,ctypes.uint32_t);
